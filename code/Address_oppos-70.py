...
def fing_addr_oppos(addr: str):
    ...
    for tx in As_input_tx - overlap:
        ...
        for input_address in tx.inputs.address.to_list():
            ...
            if input_address.type == blocksci.address_type.nulldata or input_address.type == blocksci.address_type.nonstandard or input_address.type == blocksci.address_type.witness_unknown:
                continue
            if input_address.type == blocksci.address_type.multisig_pubkey:
        ...
        for output_address in tx.outputs.address.to_list():
            ...
            if output_address.type == blocksci.address_type.nulldata or output_address.type == blocksci.address_type.nonstandard or output_address.type == blocksci.address_type.witness_unknown:
                continue
            if output_address.type is blocksci.address_type.multisig_pubkey:
    ...
    for tx in As_output_tx - overlap:
        ...
        for output_address in tx.outputs.address.to_list():
            ...
            if output_address.type == blocksci.address_type.nulldata or output_address.type == blocksci.address_type.nonstandard or output_address.type == blocksci.address_type.witness_unknown:
                continue
            if output_address.type == blocksci.address_type.multisig_pubkey:
                out_addr = output_address.addresses[0].address_string
            else:
                try:
                    out_addr = output_address.address_string
                except:
                    logger.info(output_address.type)
                    logger.info(type(output_address))
                    exit(0)
            if out_addr == addr:
                continue
...
def extract_random_entity_one_process(addr_list: List, pid: int):
    writer = csv.writer(open("../addr_oppos/random_entity_addr_oppos-70/" + str(pid) + ".csv", "a+"), delimiter=";")
    for addr in tqdm(addr_list):
        oppos = fing_addr_oppos(addr)
        writer.writerow([addr, oppos])
...
def extract_random_entity_addr_oppos_mul_process():
    ...
    ransomware_address = ransomware_address[333333: 700000]
...
def upload_addr_oppos_folder_rocksdb():
    ...
    folder = "../addr_oppos/random_addr_1_oppos/"
...
def merge_oppos(folder_path: str)-> Set:
    ...
    writer = csv.writer(open("../addr_oppos/ransomware_addr_oppos_combined.csv", "a+"))
...
def extract_random_white_one_oppos_process(addr_list: List, pid: int):
    writer = csv.writer(open("../addr_oppos/random_addr_1_oppos_oppos/" + str(pid) + ".csv", "a+"), delimiter=";")
    for addr in tqdm(addr_list):
        oppos = fing_addr_oppos(addr)
        writer.writerow([addr, oppos])
...
def extract_random_1_oppos_oppos_mul_process():
    ...
    pid_list = []
    for i in range(60):
        pid_list.append(Process(target=extract_random_white_one_oppos_process, args=(ransomware_address[i*one_process_length: (i+1) * one_process_length],i, )))
...
if __name__ == "__main__":
    ...
    extract_random_1_oppos_oppos_mul_process()
    # extract_random_white_2_oppos_mul_process()
    # logger.info("finish")
    # test_blocksci()
    # merge_oppos("../addr_oppos/ransomware_addr_oppos/")
