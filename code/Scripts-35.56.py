...
oppo_addr_path = "../archive_data/Bitcoin/oppo_addr.db"
oppo_addr_db = rocksdb.DB(oppo_addr_path, rocksdb.Options(create_if_missing=True))
addr_oppos_path = "../archive_data/Bitcoin/addr_oppos.db"
...
def add_feature_to_list_addr(addrs: list, pid: int):
    writer = csv.writer(open("../addr_self_feature/random_addr_1_added_feature/" + str(pid) + ".csv", "a+"))
...
def load_oppo_addrs():
    ...
    for path in path_list:
        reader = csv.reader(open(path, "r"))
        for row in reader:
            oppo_addr.append(row[0])
    oppo_addr = set(oppo_addr)
    return oppo_addr


# 一次性读取四个文件
def construct_addr_oppos_db_one_time():
    oppo_addrs = load_oppo_addrs()
    # oppo_addrs = set()
    ...
    save_path = "../filtered_oppos/"

    addr_opposite_loca_path = "../nfs/tmk/www_experiment/result_csv/"
    ...
    for year in range(2014, 2022):
        ...
        for index in range(10): 
            logger.info(index)
            write_list = []
            if os.path.exists(save_path + str(year) + "/") is False:
                os.makedirs(save_path + str(year) + "/")
            
            save_file_path = save_path + str(year) + "/" + str(index) + ".csv"
            writer = csv.writer(open(save_file_path, "a+"), delimiter=';')

            file_open_list = []
            for file in file_name_list:
                file_open_list.append(open(addr_opposite_loca_path + str(year) + "/" + str(index) + "/" + file, "r"))
            cnt = 0
            while True:
                if cnt % 1000000 == 0:
                    logger.info(cnt)
                ...
                # if address not in oppo_addrs:
                #     continue
                opposite_addr_list.append(eval(in_neighbor_line.split(";")[1]))
                opposite_addr_list.append(eval(out_neighbor_line.split(";")[1]))
                opposite_addr_list.append(eval(out_sib_line.split(";")[1]))
                opposite_addr_list.append(eval(in_sib_line.split(";")[1]))
                write_list.append([address, opposite_addr_list])
            for item in tqdm(write_list): 
                writer.writerow(item)
                # addr_oppos_db.put(bytes(addr, encoding='utf-8'), bytes(str(opposite_addr_list), encoding='utf-8'))
...
def construct_addr_oppos_db_using_old_data():
    oppo_addrs = load_oppo_addrs()
    logger.info("load oppo addr finish")
    save_path = "../filtered_oppos/"
    # addr_oppos_db_path = "../addr_oppos/addr_oppos_old.db"
    # addr_oppos_db = rocksdb.DB(addr_oppos_db_path, rocksdb.Options(create_if_missing=True))
    folder = "../nfs/tmk/www_experiment/result_csv/"
    role_list = ["in_neighbor.csv", "out_neighbor.csv", "out_sib.csv", "in_sib.csv"]
    for year in range(2014, 2022):
        logger.info(year)
        year_path = folder + str(year) + "/"
        for index in range(10):
            # if year == 2014 and index < 6:
            #     continue
            logger.info(index)
            if os.path.exists(save_path + str(year) + "/") is False:
                os.makedirs(save_path + str(year) + "/")
            ...
            for role in role_list:
                ...
                for row in reader:
                    ...
                    # count += 1
                    addr = row[0]
                    if addr not in oppo_addrs:
                        continue
                    count += 1
                    if addr not in addr_oppos:
                        addr_oppos[addr] = list()
                    addr_oppos[addr].append(eval(row[1]))
                logger.info(count)

            for addr, oppos in tqdm(addr_oppos.items()):
            ...
            #     batch.put(bytes(addr, encoding='utf-8'), bytes(str(oppos), encoding='utf-8'))
            #     write_count += 1
            #     if write_count == 100000:
            #         addr_oppos_db.write(batch)
            #         batch = rocksdb.WriteBatch()
            # addr_oppos_db.write(batch)
                # logger.info(addr)
                # logger.info(oppos)
...
# 将oppo_addr放入rocksdb
def construct_oppo_addr_db():
    oppo_addr_path = "../archive_data/Bitcoin/oppo_addr.db"
    oppo_addr_db = rocksdb.DB(oppo_addr_path, rocksdb.Options(create_if_missing=True))
    path_list = ["../nfs/wk/TDSC_major_revision/data/addr_oppos/random_addr_1_oppos_combined.csv", 
    "../nfs/wk/TDSC_major_revision/data/addr_oppos/random_addr_2_oppos_combined.csv", 
    "../nfs/wk/TDSC_major_revision/data/addr_oppos/random_entity_addr_oppos_combined.csv"]
    for path in path_list:
        reader = csv.reader(open(path, "r"))
        for row in reader:
            oppo_addr_db.put(bytes(row[0], encoding='utf-8'), bytes("exist", encoding='utf-8'))


# 每个index一个进程来过滤，一次性读取
def filter_oppos_addr_one_time(year, index):
    save_path = "../archive_data/Bitcoin/filtered_oppos/"
    if os.path.exists(save_path + str(year) + "/") is False:
        os.makedirs(save_path + str(year) + "/")
    save_file_path = save_path + str(year) + "/" + str(index) + ".csv"
    writer = csv.writer(open(save_file_path, "a+"), delimiter=';')
    addr_opposite_loca_path = "../archive_data/Bitcoin/result_csv/"
    file_name_list = ["in_neighbor.csv", "out_neighbor.csv", "in_sib.csv", "out_sib.csv"]
    file_open_list = []
    for file in file_name_list:
        file_open_list.append(open(addr_opposite_loca_path + str(year) + "/" + str(index) + "/" + file, "r"))
    ...
    while True:
        if cnt % 100000 == 0:
            logger.info(str(year) + ":" + str(index) + ":" + str(cnt))
        cnt += 1
        opposite_addr_list = []
        in_neighbor_line = file_open_list[0].readline()
        out_neighbor_line = file_open_list[1].readline()
        ...
        if in_neighbor_line == "":
            break
        address = in_neighbor_line.split(";")[0]
        result = oppo_addr_db.get(bytes(address, encoding='utf-8'))
        if result == None:
            continue
        # if address not in oppo_addrs:
        #     continue
        ...
        opposite_addr_list.append(eval(out_sib_line.split(";")[1]))
        opposite_addr_list.append(eval(in_sib_line.split(";")[1]))
        writer.writerow([address, opposite_addr_list])
        # write_list.append([address, opposite_addr_list])
    # for item in tqdm(write_list): 
    #     writer.writerow(item)
...
# 每个index一个进程来过滤
def filter_oppos_addr_each_process(year, index):
    role_list = ["in_neighbor.csv", "out_neighbor.csv", "out_sib.csv", "in_sib.csv"]
    addr_oppo_folder = "../archive_data/Bitcoin/result_csv/"
    index_folder = addr_oppo_folder + str(year) + "/" + str(index) + "/"
    addr_oppos = dict()
    for role in role_list:
        logger.info(str(index) + "   :   "  + role)
        role_path = index_folder + role
        reader = csv.reader(open(role_path, "r"), delimiter=';')
        count = 0
        for row in reader:
            addr = row[0]
            if count % 100000 == 0:
                logger.info(str(year) + ":"+ str(index) + ":" + str(count))
            count += 1
            result = oppo_addr_db.get(bytes(addr, encoding='utf-8'))
            if result is None:
                continue
...
# 多进程过滤麦可做的对手地址结果
def filter_addr_oppos_mul_process():
    
    for year in range(2014, 2015):
        logger.info(year)
        pid_list = []
        for i in range(10):
            pid_list.append(Process(target=filter_oppos_addr_one_time, args=(year, i, )))
...
# 将过滤出来的对手地址存入rocksdb. 数据量太大，无法写入。
def store_addr_oppos():
    filter_oppos_path = "../archive_data/Bitcoin/filtered_oppos/"
    
    for year in range(2014, 2022):
        logger.info(year)
        file_list = sorted(os.listdir(filter_oppos_path + str(year)))
        for file_name in file_list:
            logger.info(file_name)
            file_path = filter_oppos_path + str(year) + "/" + file_name
            ...
            for row in reader:
                ...
                if count % 10000 == 0:
                    addr_oppos_db.write(batch)
                    batch = rocksdb.WriteBatch()
                batch.put(bytes(addr, encoding='utf-8'), bytes(oppos_list, encoding='utf-8'))
            addr_oppos_db.write(batch)


def statistic_oppo_addr(year, index):
    filter_oppos_path = "../archive_data/Bitcoin/filtered_oppos/"

    save_folder = "../archive_data/Bitcoin/addr_oppo_num/"

    if os.path.exists(save_folder + str(year) + "/") is False:
                os.makedirs(save_folder + str(year) + "/")
    writer = csv.writer(open(save_folder + str(year) + "/" + str(index) + ".csv", "a+"))

    file_path = filter_oppos_path + str(year) + "/" + str(index) + ".csv"
    reader = csv.reader(open(file_path, "r"), delimiter=';')
    ...
    for row in reader:
        ...
        addr = row[0]
        oppos_list = eval(row[1])
        addr_oppos_num = [addr]
        for oppos in oppos_list:
            addr_oppos_num.append(len(oppos))
        write_list.append(addr_oppos_num)

        count += 1
    writer.writerows(write_list)

# 直接从原始文件统计地址的对手地址特征。
def statistic_oppo_addr_mul_process():
    
    for year in range(2014, 2022):
        pid_list = []
        for i in range(10):
            pid_list.append(Process(target=statistic_oppo_addr, args=(year, i,)))
...
if __name__ == '__main__':
    ...
    statistic_oppo_addr_mul_process()
    end_time = time.time()
