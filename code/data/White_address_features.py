...
def extract_white_address_features(addr_list, p_id):
    f = open("../nfs/wk/TDSC/addr_self_feature/white/2019/" + str(p_id) + ".csv", "a+")
    ...
    for one_addr in addr_list:
        ...
        if len(transaction_time_list) == 0:
            logger.info("number of txs is zero:" + one_addr)
            continue
...
if __name__ == '__main__':
    ...
    white_address_list = []
    white_address_path = "../nfs/wk/TDSC/random_white_address/white_2019.csv"
    ...
    for i in range(10):
        if i == 9:
            process_i = Process(target=extract_white_address_features,
                        args=(white_address_list[i*process_length :], i,))
            process_list.append(process_i)
        else:
            process_i = Process(target=extract_white_address_features,
                        args=(white_address_list[i*process_length: (i+1)*process_length], i,))
            process_list.append(process_i)
