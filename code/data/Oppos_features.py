...
def extract_white_address_features(addr_list, p_id):
    f = open("../nfs/wk/TDSC/addr_oppos_feature/ransom/0" + str(p_id) + ".csv", "a+")
    ...
    for one_addr in addr_list:
        ...
        if cnt % 1000 == 0:
            logger.info(str(p_id) + '------' + str(cnt) + ' /// ' + str(len(addr_list)))
        ...
        csv_writer.writerow([one_addr, transaction_count, receive_transaction_count, receive_transaction_amount,
...
if __name__ == '__main__':
    ...
    logger.info("mixing_transaction_set already")
    ...
    for row in oppos_address_reader:
        addr = row[0]
        oppos_list.append(addr)

    length = len(oppos_list)


    process_length = length // 10

    oppos_list = oppos_list[9 * process_length:]
    ...
    for i in range(10):
        if i == 9:
            process_i = Process(target=extract_white_address_features,
                                args=(oppos_list[i * process_length:], i,))
            process_list.append(process_i)
        else:
            process_i = Process(target=extract_white_address_features,
                                args=(oppos_list[i*process_length: (i+1) * process_length], i,))
            process_list.append(process_i)
    ...
    # oppos_address_path = "../nfs/wk/TDSC/white_address_oppos/year_random_white_oppos.csv"
    # oppos_address_f = open(oppos_address_path, "r")
    # oppos_address_reader = csv.reader(oppos_address_f)
    ...
    # start_loop = 0
    # logger.info("start_loop:" + str(start_loop))
    # loop_length = 2000000
    # logger.info("loop_length:" + str(loop_length))
    # end_loop = 55
    # logger.info("end_loop:" + str(end_loop))
    # count = 0
    # count_beacon = 110000000
    # for current_loop in range(end_loop, 110):
    #     logger.info("current_loop:" + str(current_loop))
    #     oppos_list = []
    #     for row in oppos_address_reader:
    #         if count < count_beacon:
    #             count += 1
    #             continue
    #         if count < (current_loop + 1) * loop_length:
    #             oppos_list.append(row[0])
    #         else:
    #             break
    #         count += 1


    #     oppos_set = set(oppos_list)
    #     oppos_set = oppos_set - already_address_set

    #     logger.info("oppos_set already")

    #     oppos_list = list(oppos_set)
    #     length = len(oppos_list)


    #     process_length = length // 20
    ...
    # extract_white_address_features()