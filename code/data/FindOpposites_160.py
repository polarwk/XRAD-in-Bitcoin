...
def process_multi_oppo_year(year):
    cnt = 0
    if os.path.exists('../nfs/tmk/www_experiment/result_csv/2019/' + year) is False:
        os.makedirs('../nfs/tmk/www_experiment/result_csv/2019/' + year)
    f1 = open('../nfs/tmk/www_experiment/result_csv/2019/' + year + '/in_neighbor.csv','a')
    f2 = open('../nfs/tmk/www_experiment/result_csv/2019/' + year + '/out_neighbor.csv','a')
    f3 = open('../nfs/tmk/www_experiment/result_csv/2019/' + year + '/in_sib.csv','a')
    f4 = open('../nfs/tmk/www_experiment/result_csv/2019/' + year + '/out_sib.csv','a')
    for line in open('../nfs/tmk/www_experiment/resource_csv/2019/' + year + '.csv',
                     'r'):
        if '\n' in line:
            addr = line[0:-1]
        else:
...
if __name__ == '__main__':
    db = rocksdb.DB('../cd_data/addr_related_txs.db', rocksdb.Options(create_if_missing=True), read_only=True)
    db_tx = rocksdb.DB('../cd_data/transaction_new', rocksdb.Options(create_if_missing=True), read_only=True)
