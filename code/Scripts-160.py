...
def extract_oppo_addr_extra_one_process(pid):
    ...
    save_folder = "../addr_self_feature/entity_oppo_extra_feature/"
    save_path = save_folder + str(pid) + ".csv"

    split_file_list = sorted(os.listdir("../addr_oppos/entity_oppos_split/"))

    writer = csv.writer(open(save_path, "a+"))
    addr_list = list()
    reader = csv.reader(open("../addr_oppos/entity_oppos_split/" + split_file_list[pid], "r"))
    for row in reader:
