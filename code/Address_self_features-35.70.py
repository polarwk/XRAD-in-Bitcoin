...
def extract_white_addr_feature_one_process(addr_list, pid):
    writer = csv.writer(open("../addr_self_feature/random_entity_addr_oppos-70/" + str(pid) + ".csv", "a+"))
...
def extract_white_addr_feature_multi_process():
    ...
    addr_list = addr_list[8000000:16000000]

    logger.info(len(addr_list))
    
    process_length = len(addr_list) // 59
    p_list = []
    for i in range(60):
