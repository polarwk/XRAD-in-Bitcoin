...
def compute_random_1_cascade_features_mul_process():
    ...
    ransomware_addr = ransomware_addr[0:333333]
...
def compute_random_2_cascade_features(addr_list: list, pid: int):
    writer = csv.writer(open("../addr_cascade_feature/random_addr_2_177/"+ str(pid) + ".csv", "a+"))
    for addr in tqdm(addr_list):
        cascade_features = cascade_feature_calculation(addr)
        writer.writerow(cascade_features)
...
def compute_random_entity_cascade_features(addr_list: list, pid: int):
    writer = csv.writer(open("../addr_cascade_feature/random_entity_addr_177/"+ str(pid) + ".csv", "a+"))
    for addr in tqdm(addr_list):
        cascade_features = cascade_feature_calculation(addr)
        writer.writerow(cascade_features)
...
def compute_one_addr_cascade_feature_upload(addr: str):
    cascade_features = cascade_feature_calculation(addr)
    logger.info(cascade_features)
    cascade_features.insert(1, "CryptoLocker")
    writer = csv.writer(open("../addr_cascade_feature/ransomware_addr_cascade_feature_with_label.csv", "a+"))
    writer.writerow(cascade_features)

def compute_extra_addr_from_clustering_cascade_feature_upload():
    writer = csv.writer(open(
        "../addr_cascade_feature/rs_addr_extra_from_clustering.csv",
        "a+"))
    extra_ransomware_addr_path = "../ransomware_dataset/rs_addr_extra_from_clustering.csv"
    reader = csv.reader(open(extra_ransomware_addr_path, "r"))
    for row in reader:
        addr = row[0]
        label = row[1]
        cascade_features = cascade_feature_calculation(addr)
        logger.info(cascade_features)
        cascade_features.insert(1, label)
...
if __name__ == "__main__":
    ...
    # compute_random_1_cascade_features_mul_process()
    compute_extra_addr_from_clustering_cascade_feature_upload()
    # compute_one_addr_cascade_feature_upload("12i8GBDDLx93AnCdsGQTdN46EcDAz39vXa")
    logger.info("finish")
    end_time = time.time()
